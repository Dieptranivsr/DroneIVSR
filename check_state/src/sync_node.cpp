...
private:
    void checkCb(const sensor_msgs::ImageConstPtr& img, const geometry_msgs::PoseStampedConstPtr& pose);
...
void Syn::initSyn(ros::NodeHandle& nh){
    ...
    depthImg =
      node.subscribe<geometry_msgs::PoseStamped>("/camera/depth/image_rect_raw", 10, &Syn::odomCallback, this);
    pose =
      node.subscribe<geometry_msgs::PoseStamped>("/camera/pose", 10, &Syn::airSimCallback, this);
     
}

void Syn::checkCb(const sensor_msgs::ImageConstPtr& img, const geometry_msgs::PoseStampedConstPtr& pose){
    std::cout << "HIIIIIIIIIIIIIIIII" << std::endl;
}

void Syn::odomCallback(const sensor_msgs::ImageConstPtr& img) {

}

void Syn::airSimCallback(const geometry_msgs::PoseStampedConstPtr& pose) {

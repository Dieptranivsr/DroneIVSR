...
void gazeboCallback(const gazebo_msgs::ModelStates::ConstPtr& gazebo_model_msg){
     ...
     if(!first_received){ // For the first time find required model name (index)
       while(!first_received){
         if(!gazebo_model_msg->name[i].compare("iris") or !gazebo_model_msg->name[i].compare("d9")){
           index_name = i;
           first_received = true;
...
public:
    ...
    FakeMocapPX4(int argc, char** argv){
        ...
        // Subscribers
        mocap_subscriber = td.subscribe("/in_odometry", 1, gazeboCallback);
        //mocap_subscriber = node_handle.subscribe("/gazebo/model_states", 1, gazeboCallback);

        // Publishers
        mocap_publisher = td.advertise<geometry_msgs::PoseStamped>("/out_odometry", 1);
...
int main(int argc, char** argv){
    std::cout << "[FakeMocapPX4] FakeMocapPX4 is running..." << std::endl;

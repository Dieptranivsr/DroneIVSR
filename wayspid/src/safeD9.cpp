...
geometry_msgs::Twist filter(){
    ...
    t.angular.y = 0;
    t.angular.z = 0;
    for(std::vector<geometry_msgs::Twist>::iterator i = velocities.begin(); i != velocities.end(); ++i){
 ...
 */
void dynamicReconfigureCallback(wayspid::setSafeD9Config &config, uint32_t level){
	std::cout << "[SafeD9] setSafeD9Config is running..." << std::endl;
...
void optitrackCallback(const geometry_msgs::PoseStamped& optitrack_msg){
    ros::Duration dt = optitrack_msg.header.stamp - time_old;
    ...
    odometry_msg.child_frame_id = "D9";
    ...
    noisy_odometry_pub.publish(odometry_msg);
...
// Constructor
SafeD9::SafeD9(int argc, char** argv){
    ros::init(argc, argv, "safeD9");
    ros::NodeHandle td;

    // Subscribers
    optitrack_sub = td.subscribe("/optitrack/odometry", 1, optitrackCallback);
    command_sub = td.subscribe("/d9/command", 1, commandCallback);
    command_position_sub = td.subscribe("/d9/command_position", 1, commandPositionCallback);
    command_velocity_sub = td.subscribe("/d9/command_velocity", 1, commandVelocityCallback);
    command_attitude_sub = td.subscribe("/d9/command_attitude", 1, commandAttitudeCallback);
    noise_sub = td.subscribe("/d9/noise", 1, noiseCallback);
    battery_sub = td.subscribe("/mavros/battery", 1, batteryCallback);
    state_sub = td.subscribe<mavros_msgs::State>("mavros/state", 10, stateCallback);
    velocity_actual_sub = td.subscribe<geometry_msgs::TwistStamped>("/mavros/local_position/velocity",1,actualVelocityCallback);
    local_pos_sub = td.subscribe<geometry_msgs::PoseStamped>("mavros/local_position/pose", 10, localPosCallback);

    // Publishers
    position_pub = td.advertise<geometry_msgs::PoseStamped>("/mavros/setpoint_position/local", 1);
    velocity_pub = td.advertise<geometry_msgs::TwistStamped>("/mavros/setpoint_velocity/cmd_vel", 1);
    attitude_pub = td.advertise<geometry_msgs::PoseStamped>("/mavros/setpoint_attitude/target_attitude", 1);
    throttle_pub = td.advertise<mavros_msgs::Thrust>("/mavros/setpoint_attitude/thrust", 1);
    odometry_pub = td.advertise<nav_msgs::Odometry>("/d9/ground_truth/odometry", 1);
    noisy_odometry_pub = td.advertise<nav_msgs::Odometry>("/d9/odometry", 1);
    test_msg_pub = td.advertise<std_msgs::Float64>("/test_thrust",1);
    att_com_pub = td.advertise<geometry_msgs::PoseStamped>("/attitude_RPY/command", 1);
    att_local_pub = td.advertise<geometry_msgs::PoseStamped>("/attitude_RPY/local", 1);

    raw_pos_com_pub = td.advertise<mavros_msgs::PositionTarget>("/mavros/setpoint_raw/local", 1);
    raw_att_com_pub = td.advertise<mavros_msgs::AttitudeTarget>("/mavros/setpoint_raw/attitude", 1);

    // ROS service clients
    set_mode_client = td.serviceClient<mavros_msgs::SetMode>("mavros/set_mode");
    arming_client = td.serviceClient<mavros_msgs::CommandBool>("mavros/cmd/arming");
    land_client = td.serviceClient<mavros_msgs::CommandTOL>("mavros/cmd/land");
...
void SafeD9::run(){
    ...
    dynamic_reconfigure::Server<wayspid::setSafeD9Config> server;
    dynamic_reconfigure::Server<wayspid::setSafeD9Config>::CallbackType f;
    ...
    while(ros::ok()){
        ...
        att_local_pub.publish(att_local_eul);
        ...
        // ********************** Attitude setpoint **********************
            ...
            if(position.x < MAX_X && position.x > -MAX_X && position.y < MAX_Y && position.y > -MAX_Y && position.z < MAX_Z && position.z > -MAX_Z && !stop){
                ...
                att_com_pub.publish(att_com_eul);
                ...
                */
                ...
                test_msg_pub.publish(test_thrust);
         ...
         * */
          ...
          // Position and velocity command
            ...
            // Attitude command
              ...
              raw_pos_com_pub.publish(raw_target_pos_msg);
...
int main(int argc, char** argv){
    std::cout << "[SafeD9] SafeD9 is running..." << std::endl;

    SafeD9* d9 = new SafeD9(argc, argv);

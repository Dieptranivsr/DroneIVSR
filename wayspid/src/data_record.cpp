#include <wayspid/data_record.h>

// Set the maximum record time to 100s by default
double max_rec_time = 100;

void dynamicReconfigureCallback(wayspid::setDataRecordConfig &config, uint32_t level)
{
	std::cout << "[data_record] setDataRecordConfig is running..." << std::endl;
...
data_record::data_record(int argc, char** argv){
    ...
    // Publisher
    record_flag_pub = td.advertise<std_msgs::Int32>("/record_start_flag", 1);

    // Subscribers
    traj_type_sub = td.subscribe<std_msgs::Int32>("/trajectory_type", 1, trajTypeCallback);
    motor_num_sub = td.subscribe<std_msgs::Int32>("/motor_failure/motor_number", 1, motorNumCallback);

    local_pos_sub = td.subscribe<geometry_msgs::PoseStamped>("mavros/local_position/pose", 1, localPosCallback);
    ref_traj_sub = td.subscribe<geometry_msgs::QuaternionStamped>("/d9/command_position", 1,refPosCallback);
    local_att_sub = td.subscribe<geometry_msgs::PoseStamped>("/attitude_RPY/local", 1, localAttitudeCallback);
    com_att_sub = td.subscribe<geometry_msgs::PoseStamped>("/attitude_RPY/command", 1, comAttitudeCallback);
    local_vel_sub = td.subscribe<geometry_msgs::TwistStamped>("mavros/local_position/velocity", 1, localVelCallback);
    com_vel_sub = td.subscribe<geometry_msgs::Quaternion>("/d9/command_velocity", 1, comVelCallback);

    pd_vel_sub = td.subscribe<geometry_msgs::Quaternion>("/d9/command/velocity_pd", 1,pdVelCallback);

    com_thr_sub = td.subscribe<std_msgs::Float64>("/test_thrust",1,comThrCallback);
...
void data_record::run(){
    ...
    dynamic_reconfigure::Server<wayspid::setDataRecordConfig> server;
    dynamic_reconfigure::Server<wayspid::setDataRecordConfig>::CallbackType f;
    ...
    std::string path_dir = ros::package::getPath("controllers") + "/Data/";
    std::string tmpfile = "tempData.txt";
 ...
 *      std::cout<<rec_time<<std::endl;     */
        ...
        if(record_start && traj_type == 6){ // ************** Modify here **************
            ...
            outputFile<<ros::Time::now()<<" "                                                       // column 1:      time
                      ...
                      <<comThrust_<< std::endl;                                                           // column 30:     command throttle
...
int main(int argc, char** argv){
	std::cout << "[record_data] Data logger is running..." << std::endl;
